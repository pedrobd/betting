...
export async function getDailyMatches() {
    ...
    try {
        ...
        // Deep Scrape para buscar odds reais e FORMA
        if (validMatches.length < 20) {
            console.log("🔍 Iniciando Deep Scrape...");
            const candidates = needsDeepScrape.length > 0 ? needsDeepScrape : futurematches;
            let missingOdds = candidates.slice(0, 30);
            ...
            for (let m of missingOdds) {
                ...
                if (validMatches.length >= 25) break;
            }
        }
