    ...
    try {
        ...
        for (let m of deepQueue) {
            ...
            try {
                ...
                const matchIntel = await detailPage.evaluate(() => {
                    const getForm = (sectionIdx) => {
                        const sections = document.querySelectorAll('.h2h__section');
                        if (!sections[sectionIdx]) return "";
                        const icons = sections[sectionIdx].querySelectorAll(
                            '.h2h__result, .h2h__icon, [class*="wld"], [class*="result"]'
                        );
                        return Array.from(icons).slice(0, 5).map(icon => {
                            const cls = icon.className.toLowerCase();
                            // PT: V=Vitória(W), E=Empate(D), D=Derrota(L)
                            // EN: W=Win, D=Draw, L=Loss
                            // CSS: positive=W, draw=D, negative=L
                            const text = (icon.innerText || icon.textContent || '').toUpperCase().trim();
                            if (cls.includes('positive') || text === 'V' || text === 'W') return 'W';
                            if (cls.includes('draw')     || text === 'E') return 'D';
                            if (cls.includes('negative') || text === 'D' || text === 'L') return 'L';
                            return '';
                        }).filter(Boolean).join('');
                    };
